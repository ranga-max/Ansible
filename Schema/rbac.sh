...
mip=192.168.75.7
confluent cluster unregister --cluster-name "SrDev" 
...
confluent iam rbac role-binding create --principal User:mds --role ClusterAdmin --kafka-cluster-id $cluster_id
confluent iam rbac role-binding create --principal User:c3 --role SystemAdmin --kafka-cluster-id $cluster_id
#confluent iam rbac role-binding create --principal User:c3 --role SystemAdmin --kafka-cluster-id  $cluster_id  --schema-registry-cluster-id "id_schemaregistry_confluent"
...
#confluent iam rbac role-binding create --principal User:sr --role SystemAdmin --kafka-cluster-id  $cluster_id  --schema-registry-cluster-id "id_schemaregistry_confluent"
confluent iam rbac role-binding create --principal User:c3 --role SystemAdmin --kafka-cluster-id  $cluster_id  --schema-registry-cluster-id "schema-registry"
